...
import {SlideViewer} from 'astro-nebula-ui';
...
The **Slide Viewer** is an Astro component that automatically converts a website's page content into an interactive slide presentation system. It provides a rich set of features for navigation, customization, and content interaction through a modular architecture.
...
The SlideViewer component supports extensive configuration for different Astro site layouts and content structures beyond Starlight.
...
<SlideViewer containerSelector=".my-content-area" />

<!-- Multiple possible containers -->
<SlideViewer containerSelector=".main-content, .article-body, #content" />
...
<SlideViewer slideBreakers="h2" />

<!-- Custom class-based breaks -->
<SlideViewer slideBreakers="h2, .slide-break, .new-slide" />

<!-- Multiple element types -->
<SlideViewer slideBreakers="hr, h1, h2, .chapter-break" />
```
...
<!-- Custom exclusions -->
<SlideViewer excludeElements="script, style, .sidebar, .navigation" />

<!-- Exclude ads and widgets -->
<SlideViewer excludeElements="script, style, .ads, .widget, .social-share" />

<!-- Minimal exclusions -->
<SlideViewer excludeElements="script, style" />
```
