...
export class SlideViewer {
  constructor() {
    // Lightweight initialization - only essential properties
    this.slides = [];
    this.currentSlide = 0;
    ...
    this.isFullyInitialized = false; // Flag to track lazy initialization
    ...
    // Only get essential elements for initial setup
    this.modal = document.getElementById("slide-viewer-modal");
    this.startBtn = document.getElementById("start-slideshow");

    // Read configuration from data attributes with fallbacks
    this.config = {
      containerSelector:
        this.modal?.dataset.containerSelector ||
        ".sl-markdown-content, main .content, article",
      slideBreakers: this.modal?.dataset.slideBreakers || "hr, h1, h2, h3",
      excludeElements:
    ...
    // Only bind the start button initially - everything else is lazy loaded
    this.bindStartButton();
  }

  // Validate and normalize configuration
  validateConfig() {
    // Ensure containerSelector is not empty
    if (!this.config.containerSelector?.trim()) {
      console.warn("SlideViewer: Empty containerSelector, using default");
      this.config.containerSelector =
        ".sl-markdown-content, main .content, article";
    }

    // Ensure slideBreakers is not empty
    if (!this.config.slideBreakers?.trim()) {
      console.warn("SlideViewer: Empty slideBreakers, using default");
      this.config.slideBreakers = "hr, h1, h2, h3";
    }

    // Normalize excludeElements (can be empty)
    this.config.excludeElements = this.config.excludeElements?.trim() || "";

    // console.log("SlideViewer configuration:", this.config);
  }
  ...
  isElementAllowed(element) {
    ...
    // Element is not allowed if it's in the blacklist
    if (this.blacklistedSlideElements.has(tagName)) {
      return false;
    }

    // Element is not allowed if it matches excludeElements selector
    if (
      this.config.excludeElements &&
      element.matches &&
      element.matches(this.config.excludeElements)
    ) {
      return false;
    }
  ...
  setupAccessibility() {
    ...
    if (!document.getElementById("slideshow-description")) {
      ...
      description.textContent =
        "Interactive slideshow viewer. Use arrow keys to navigate, Escape to close.";
    ...
    if (this.laserPointerBtn) {
      this.laserPointerBtn.setAttribute(
        "aria-label",
    ...
    if (this.laserPointerBtnMobile) {
      this.laserPointerBtnMobile.setAttribute(
        "aria-label",
    ...
    const controlsContainer = this.modal.querySelector(
      ".slide-controls, .fb-slide__controls"
    );
    ...
    if (this.gotoInput) {
      this.gotoInput.setAttribute(
        "aria-label",
        "Enter slide number to navigate to"
      );
    ...
    if (this.progressFill && this.progressFill.parentElement) {
      ...
      this.progressFill.parentElement.setAttribute(
        "aria-label",
        "Slide progress"
      );
  ...
  updateFocusableElements() {
    ...
    this.focusableElements = Array.from(
      ...
      return (
        el.offsetWidth > 0 && el.offsetHeight > 0 && !el.closest(".hidden")
      );
    });
  ...
  updateGotoModalFocusableElements() {
    ...
    this.gotoModalLastFocusable =
      this.gotoModalFocusableElements[
        this.gotoModalFocusableElements.length - 1
      ];
  }
  ...
  // Lightweight method to only bind the start button
  bindStartButton() {
    if (this.startBtn) {
      this.startBtn.addEventListener("click", () => {
        // console.log("Start button clicked - initializing slideshow");
        this.openSlideshow();
      });
    }
  }

  // Heavy initialization moved here - only called when slideshow is actually opened
  initializeOnDemand() {
    if (this.isFullyInitialized) {
      return; // Already initialized
    }

    //console.log("Performing full SlideViewer initialization...");

    // Get all UI elements
    this.slideContent = document.getElementById("slide-content");
    this.totalSlidesEl = document.getElementById("total-slides");
    this.slideNumberIndicator = document.getElementById(
    ...
    this.laserPointerBtnMobile = document.getElementById(
      "toggle-laser-pointer-mobile"
    );
    ...
    // Parse slides and setup everything
    this.parseSlides();
    this.bindEvents();
    this.setupAccessibility();

    // Hide start button if no slides found
    if (this.slides.length === 0) {
      this.startBtn.style.display = "none";
    }

    this.isFullyInitialized = true;
    // console.log("Full SlideViewer initialization complete");
  }

  parseSlides() {
    let articleContent = document.querySelector(this.config.containerSelector);

    // If the custom selector fails and Starlight is enabled, try fallback selectors
    if (!articleContent && this.config.includeStarlight) {
      console.warn(
        `SlideViewer: Could not find content using selector: ${this.config.containerSelector}`
      );
      // console.log("SlideViewer: Trying fallback selectors...");
      ...
      for (const selector of fallbackSelectors) {
        ...
        if (articleContent) {
          // console.log(
          //   `SlideViewer: Found content using fallback selector: ${selector}`
          // );
          break;
        }
      }
    ...
    if (!articleContent) {
      console.error(
        `SlideViewer: Could not find article content using any selector. Tried: ${this.config.containerSelector}`
      );
      return;
    }
    ...
    const tocElements = content.querySelectorAll(
      ".fb-slide__toc-sidebar, .fb-slide__toc-header"
    );
    ...
    // Remove excluded elements based on configuration
    if (this.config.excludeElements) {
      const excludedElements = content.querySelectorAll(
        this.config.excludeElements
      );
      excludedElements.forEach((el) => {
        el.remove();
        // console.log(`Removed excluded element: ${el.tagName}`);
      });
    }

    const slideBreakElements = content.querySelectorAll(
      this.config.slideBreakers
    );
    // console.log(
    //   `Found slide break elements (${this.config.slideBreakers}):`,
    //   slideBreakElements.length
    // );

    if (slideBreakElements.length === 0) {
      // No separators found, treat entire content as one slide
      this.slides = [content];
    } else {
      // Split content by configured slide break elements
      this.slides = [];
      let currentSlideContent = document.createElement("div");

      // Create a set of slide break tag names for efficient lookup
      const slideBreakTagNames = new Set();
      this.config.slideBreakers.split(",").forEach((selector) => {
        const trimmed = selector.trim();
        // Handle simple tag selectors (hr, h1, h2, etc.)
        if (/^[a-z][a-z0-9]*$/i.test(trimmed)) {
          slideBreakTagNames.add(trimmed.toUpperCase());
        }
      });
      ...
      Array.from(content.childNodes).forEach((node) => {
        const isSlideBreak =
          ...
          (slideBreakTagNames.has(node.tagName) ||
            (node.matches && node.matches(this.config.slideBreakers)));

        if (isSlideBreak) {
          // Only add slides that have allowed HTML elements
          if (currentSlideContent.children.length > 0) {
            const hasAllowedContent = Array.from(
              currentSlideContent.children
            ).some((child) => this.isElementAllowed(child));
          ...
          // For headings, include them in the new slide; for HR and other elements, don't include them.
      ...
      if (currentSlideContent.children.length > 0) {
        const hasAllowedContent = Array.from(currentSlideContent.children).some(
          (child) => this.isElementAllowed(child)
        );
    ...
    /* "Slides content preview:",
      this.slides.map((slide, index) => ({
        ...
        firstHeading: slide.querySelector("h1, h2, h3, h4, h5, h6")
          ? slide.querySelector("h1, h2, h3, h4, h5, h6").textContent.trim()
          : "no heading",
  ...
  bindEvents() {
    ...
    // Start button is now handled in bindStartButton() method
    // this.startBtn.addEventListener('click', () => {
    ...
    document.addEventListener("keydown", (event) => {
      if (!this.modal.classList.contains("hidden")) {
        ...
        if (
          (this.searchManager.searchInput &&
            document.activeElement === this.searchManager.searchInput) ||
          (this.gotoInput && document.activeElement === this.gotoInput) ||
          (this.searchManager.searchModalInput &&
            document.activeElement === this.searchManager.searchModalInput)
        ) {
          return;
        }
        ...
        if (this.isReadingMode) {
          ...
          if (event.key === "Escape") {
            ...
            (event.key === "/" || event.key === "f" || event.key === "F") &&
            this.searchManager.searchModal.classList.contains("hidden") &&
            this.gotoModal.classList.contains("hidden")
          ) {
            event.preventDefault();
    ...
    this.slideCounter.addEventListener("click", (event) => {
      if (
        event.target === this.slideCounter ||
        this.slideCounter.contains(event.target)
      ) {
    ...
    this.gotoModal.addEventListener("click", (event) => {
      ...
      if (
        event.target === this.gotoModal ||
        event.target.closest(".fb-slide__goto-modal") === this.gotoModal
      ) {
        this.closeGotoModal();
      }
    });
  ...
  closeSlideshow() {
    ...
    if (
      this.searchManager.searchInput &&
      this.searchManager.searchInput.value.trim()
    ) {
  ...
  goToSlideFromInput(inputElement) {
    ...
    if (
      ...
      console.log(
        `Invalid slide number: ${inputElement.value}. Must be between 1 and ${this.slides.length}`
      );

      // Add error styling using CSS classes.
      const isSlideInput = inputElement.classList.contains("fb-slide__input");
      const errorClass = isSlideInput
      ...
      const existingTooltip =
        inputElement.parentElement.querySelector(".error-tooltip");
  ...
  openGotoModal() {
    ...
    this.gotoModal.addEventListener(
      "keydown",
      this.trapGotoModalFocus.bind(this)
    );
  ...
  closeGotoModal() {
    ...
    // Remove focus trap.
    this.gotoModal.removeEventListener(
      "keydown",
      this.trapGotoModalFocus.bind(this)
    );
